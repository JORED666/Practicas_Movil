...
fun AddStudentScreen(navController: NavController, studentViewModel: StudentViewModel) {
    ...
    Column(
        ...
        Text(text = "Add New Student", modifier = Modifier.padding(bottom = 16.dp))
        OutlinedTextField(
            value = name,
        ...
        Button(
            onClick = {
                val newStudent = Student(
                    id = (studentViewModel.studentsUiState.value.maxOfOrNull { it.id } ?: 0) + 1,
                    name = name,
                    description = description,
                    imageUrl = imageUrl.ifEmpty { "https://picsum.photos/seed/${System.currentTimeMillis()}/200/300" }
                )
